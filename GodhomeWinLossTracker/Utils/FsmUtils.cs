...
namespace GodhomeWinLossTracker.Utils
{
    internal static class FsmUtils
    {
        ...
        {
            DevUtils.Log($"PlayMakerFSM_Start: GO={self.gameObject.name} FsmName={self.FsmName}");
            orig(self);
        }

        private static void PlayMakerFSM_SetState(On.PlayMakerFSM.orig_SetState orig, PlayMakerFSM self, string stateName)
        {
            DevUtils.Log($"PlayMakerFSM_SetState: GO={self.gameObject.name} FsmName={self.FsmName} stateName={stateName}");
            orig(self, stateName);
        }

        private static void PlayMakerFSM_Update(On.PlayMakerFSM.orig_Update orig, PlayMakerFSM self)
        {
            DevUtils.Log($"PlayMakerFSM_Update: GO={self.gameObject.name} FsmName={self.FsmName}");
            orig(self);
        }

        private static void PlayMakerFSM_SendEvent(On.PlayMakerFSM.orig_SendEvent orig, PlayMakerFSM self, string eventName)
        {
            DevUtils.Log($"PlayMakerFSM_SendEvent: GO={self.gameObject.name} FsmName={self.FsmName} eventName={eventName}");
            orig(self, eventName);
        }
        ...
        {
            DevUtils.Log($"PlayMakerFSM_SendRemoteFsmEvent: GO={self.gameObject.name} FsmName={self.FsmName} eventName={eventName}");
            orig(self, eventName);
        }
        ...
        {
            DevUtils.Log($"PlayMakerFSM_ChangeState_FsmEvent: GO={self.gameObject.name} FsmName={self.FsmName} eventName={fsmEvent.Name}");
            orig(self, fsmEvent);
        }
    }
}
